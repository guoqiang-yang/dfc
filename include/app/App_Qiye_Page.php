...
class App_Qiye_Page extends App_Qiye_Web
{
	...
	function __construct($lgmode='pub', $tmplpath=QY_TPL_PATH, $cssjs=QY_HOST)
	{
	...
	protected function checkAuth()
	{
		...
		{
			header('Location: http://'.QY_HOST.'/user/login.php');
			exit;
		}
	...
	{
		$this->title = empty($this->title) ? '好材-企业号' : $this->title;

		list($module, $page) = $this->getCurrentPage();

		$this->smarty->assign('curPage', $page);
		$this->smarty->assign('curModule', $module);
		$this->smarty->assign('modules', Conf_Qiye_Page::getMODULES($this->_uid, $this->_user));
